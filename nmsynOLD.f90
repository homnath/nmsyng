...
c       specify the catalogue information
...
c       specify cmtsolution file name

      read(*,'(a)') cmt_file
      print *,' '
      print *,'    CMTSOLUTION file: ',cmt_file(1:lnblnk(cmt_file))
      call get_cmt(cmt_file,yr,jda,ho,mi,sec,t_cmt,hdr,elat,
     #             elon,depth,mrr,mtt,mpp,mrt,mrp,mtp)
      hdur=dble(hdr)

c       specify the shortest period in mode summation
...
c       specify sampliing rate and length of the record in seconds
...
c       get the radii for the upper NL layers at which the eigenfunctions
c       are stored; needed for source radius
...
c       get the source radius and index
c
...
c
c       read the station information and loop over stations
c
...
c
c           aze is measured clockwise from due North by delaz; convert this
c           to a counterclockwise measurement from due South, which is the
c           Dahlen & Tromp convention
c
          aze=180.0d0-aze
          if(aze.lt.0.0d0) aze=aze+360.0d0
          print *,'source azimuth: ',aze,' degrees (c.c. from due South)'
c
c           initialize information for Legendre calculations
c
...
c
c           initialize the three-component mode sum
c
          do i=1,nps
            do j=1,3
...
c
          ...
          do while(iom.eq.0)
            read(2,iostat=iom)
     #         n,l,om,q,av,ah,(w(i),i=1,NL),(dw(i),i=1,NL)
           print*, 'T#####', om,om_min,om_max
           if(iom.eq.0) then
              if(om.ge.om_min.and.om.le.om_max) then
c                 minos calculates and stores all the overtones for a give
c                 degree, so Legendre polynomials have to be calculated
c                 only once per degree
c
                if(l.ne.lprev) then
...
c
c                 get w and dw at the source radius rs
c
                if(r(is).eq.rs) then
                  ws=w(is)
                  dws=dw(is)
                else
                  call interpolate(r(is),w(is),dw(is),
     #                   r(is+1),w(is+1),dw(is+1),rs,ws,dws)
                endif
c
c                 Dahlen & Tromp (10.56-10.59) for toroidal modes,
c                 but with minos modes: W_DT=om*W_minos
c
...
c
                ...
                dadp=p(2)*(-a1*sinp+b1*cosp)
     #               +2.0d0*p(3)*(-a2*sin2p+b2*cos2p)
c
c                 longitudinal and transverse components
c
...
c
          ...
          do while(iom.eq.0)
            read(2,iostat=iom)
     #         n,l,om,q,av,ah,(u(i),i=1,NL),(du(i),i=1,NL)
            print*, 'R#####', om,om_min,om_max
...
c
c                 get u and du at the source radius rs
c
                if(r(is).eq.rs) then
                  us=u(is)
                  dus=du(is)
                else
                  call interpolate(r(is),u(is),du(is),
     #                   r(is+1),u(is+1),du(is+1),rs,us,dus)
                endif
c
c                 Dahlen & Tromp (10.54) for radial modes,
c                 but with minos modes: U_DT=om*U_minos
c
...
c
c                 get u, du, v, and dv at the source radius rs
c
                if(r(is).eq.rs) then
                  us=u(is)
                  dus=du(is)
                  ...
                  call interpolate(r(is),u(is),du(is),
     #                   r(is+1),u(is+1),du(is+1),rs,us,dus)
                  call interpolate(r(is),v(is),dv(is),
     #                   r(is+1),v(is+1),dv(is+1),rs,vs,dvs)
                endif
c
c                 Dahlen & Tromp (10.54-10.59) for spheroidal modes,
c                 with U_DT=om*U_minos and V_DT=om*V_minos
c
...
c
                ...
                dadt=dp(1)*a0+dp(2)*(a1*cosp+b1*sinp)
     #                      +dp(3)*(a2*cos2p+b2*sin2p)
                dadp=p(2)*(-a1*sinp+b1*cosp)
     #               +2.0d0*p(3)*(-a2*sin2p+b2*cos2p)
c
c                 vertical, longitudinal, and transverse components
c
...
c
      implicit none
c
c     interpolate finds the value of a function fs and its derivative dfs
c     at the radius rs by interpolating given values of the function and its
c     derivative just below the source at radius r1, given by f1 and df1,
c     and values just above the source ar radius r2, given by f2 and df2.
c     Note that we should have r2 > rs > r1.
c
