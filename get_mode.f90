subroutine get_mode(type,nord,l,wcom,qmod,nnl,r,u,du,v,dv,av,ah)
  ...
  integer :: iomod,iocat,iobin,nrec,ios,nn,ll,nnn,lll,junk1,junk2,junk3,junk4
  ...
  catalogue = '/opt/seismo-util/Modes/nmcat/spheroidal.aniprem808_40s'
  bin_file = '/opt/seismo-util/Modes/nmcat/spheroidal.aniprem808_40s_all.bin'
  nvec = 6 * NL 
  type1 = 'S';type2 = 's';
...
200     format(i5,1x,a1,i5,5g16.7)
    ...
    !         junk1,junk3,nnn,lll,wcom,qmod,av,ah,(buf(i),i=1,nvec),junk2,junk4
    !    close(iobin)
    ! for 40s
        open(iobin,file=bin_file,status='old',iostat=ios,form='unformatted',&
             access='direct',recl=reclen+4*2)
        if (ios .ne. 0) stop 'Error opening catalogue file'
        read(iobin,rec = nrec,iostat = ios) &
